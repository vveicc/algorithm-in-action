...
???+ note "问题描述"
    给你两个长度为 $n(1≤n≤500)$ 的整数数组 $cost$ 和 $time$ 。<br>
    现需要给 $n$ 堵墙刷油漆，你有两名油漆匠：

    - 一位需要 **付费** 的油漆匠，刷第 $i$ 堵墙需要花费 $1≤time[i]≤1e6$ 单位的时间，开销为 $1≤cost[i]≤500$ 单位的钱；
    - 一位 **免费** 的油漆匠，刷 **任意** 一堵墙的时间为 $1$ 单位，开销为 $0$ 。但是必须在付费油漆匠 **工作** 时，免费油漆匠才会工作。

    请你返回刷完 $n$ 堵墙最少开销为多少。

    在 [LeetCode主站](https://leetcode.com/problems/painting-the-walls "Hard")
    或 [力扣中文社区](https://leetcode.cn/problems/painting-the-walls "困难：2425") 查看该题。

??? info "解题思路"
    记付费刷墙个数为 $c$ ，付费刷墙时间之和为 $t$ ，题目要求：$t >= n-c$ 。<br>
    移项得：$t + c >= n$ ，把 $c$ 拆分到每个付费刷墙时间上可得：$\sum(time[i] + 1) >= n$ 。

    至此，问题转化为 0-1 背包问题：<br>
    有 $n$ 个物品，每个物品的体积为 $time[i] + 1$ ，价值为 $cost[i]$，选择体积和 **至少** 为 $n$ 的物品，价值和 **最小** 是多少？

    $dp[i][j]$ 表示只从前 $i$ 个物品中选择的情况下，选择体积和至少为 $j$ 的物品的最小价值和。<br>
    实现时可以优化掉第一个维度，注意循环枚举顺序。

    === "Go"
        ```go
        --8<-- "knapsack/01/go/lc2742.go"
        ```
